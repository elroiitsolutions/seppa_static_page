...
const ScrollEnquiryModal: React.FC = () => {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ direction: isArabic ? 'rtl' : 'ltr' }}>
        {/* Backdrop */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <button
            onClick={() => setIsOpen(false)}
            className={`absolute top-4 ${isArabic ? 'left-4' : 'right-4'} z-20 w-10 h-10 bg-white/10 hover:bg-seppa-red hover:text-white text-gray-300 rounded-full flex items-center justify-center transition-colors duration-300`}
          ...
          <div className="relative z-10 p-8 sm:p-10 text-start">
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsOpen(false); }}>
              <div className="relative">
                ...
                />
                <span className={`absolute top-3 ${isArabic ? 'left-3' : 'right-3'} text-white`}>*</span>
              </div>
