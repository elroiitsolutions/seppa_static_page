...
export default async function LocalizedGlassBottleFillerCapperPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'glasscapper' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'glasscapper' });
  ...
  const relatedBlogs = await getRelatedBlogs('/glass-bottle-filler-capper', locale);
