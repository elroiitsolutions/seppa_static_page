...
export default async function KombopurePage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'kombo' });
  const messages = await getMessages({ locale });

  const contentBlockImages = [img1.src, img2.src, img3.src, img4.src];
  ...
  const relatedBlogs = await getRelatedBlogs('/kombo', locale);
