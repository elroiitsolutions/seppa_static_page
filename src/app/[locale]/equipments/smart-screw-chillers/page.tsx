...
export default async function SmartScrewChillersPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'smartscrew' });
  const messages = await getMessages({ locale });

  // Fetch blogs related to this page automatically
  const relatedBlogs = await getRelatedBlogs('/equipments/smart-screw-chillers', locale);
  ...
  const pageData: PackagingPageData = {
    ...
    contentBlocks: (t.raw('contentBlocks') as any[] || []).map((block: any, index: number) => ({
      ...
      image1: [ "/images/equipments/chiller_overview.png", "/images/equipments/chiller_overview.png" ][index] || undefined,
