...
export default async function LocalizedServiceSparePartsPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'servicespareparts' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'servicespareparts' });
  ...
  const relatedBlogs = await getRelatedBlogs('/services/spare-parts-and-logistics-training', locale);
