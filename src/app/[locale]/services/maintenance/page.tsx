...
export default async function LocalizedServiceMaintenancePage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'servicemaintenance' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'servicemaintenance' });
  ...
  const relatedBlogs = await getRelatedBlogs('/services/maintenance', locale);
