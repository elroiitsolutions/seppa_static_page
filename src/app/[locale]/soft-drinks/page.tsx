...
import bannerImg from '@/assets/packaging/generated/softdrink_banner.png';
import overviewImg from '@/assets/packaging/generated/softdrink_overview.png';
import executeImg from '@/assets/packaging/generated/softdrink_execute.png';
import whyChooseImg from '@/assets/packaging/generated/softdrink_whychoose.png';
...
export default async function SoftDrinksPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'softdrink' });
  const messages = await getMessages({ locale });

  // Fetch blogs related to this page automatically
  const relatedBlogs = await getRelatedBlogs('/soft-drinks', locale);
  ...
  const pageData: PackagingPageData = {
      ...
      try {
        ...
        if (Array.isArray(raw)) {
          return raw.map((block: any, index: number) => ({
            ...
            image1: [overviewImg.src, executeImg.src][index] || overviewImg.src,
    ...
    methodology: {
      ...
      steps: (t.raw('methodology.steps') as any[]).map((step: any, index: number) => ({
        ...
        image: [overviewImg.src, executeImg.src][index] || overviewImg.src
      }))
    },
    faqTitle: t('faqTitle'),
    faqs: t.raw('faqs')
