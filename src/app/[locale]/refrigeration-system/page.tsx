...
export default async function LocalizedRefrigerationSystemPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'refrigeration' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'refrigeration' });
  ...
  const relatedBlogs = await getRelatedBlogs('/refrigeration-system', locale);
