...
export default async function LocalizedCrateStackerPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'stacker' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'stacker' });
  ...
  const relatedBlogs = await getRelatedBlogs('/crate-stacker', locale);
