...
export default async function LocalizedGlassBottleWasherPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations({ locale, namespace: 'glasswasher' });
  const tEn = await getTranslations({ locale: 'en', namespace: 'glasswasher' });
  ...
  const relatedBlogs = await getRelatedBlogs('/glass-bottle-washer', locale);
  ...
  const pageData: PackagingPageData = {
    ...
    contentBlocks: [
      ...
      {
        title: getT('contentBlocks.4.title'),
        paragraphs: getRaw('contentBlocks.4.paragraphs'),
        image1: img2.src,
        reverse: true
      }
    ],
