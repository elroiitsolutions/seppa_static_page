...
export default async function PharmaCosmeticsPetBlowingPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'pharmacosmeticspetblowing' });
  const messages = await getMessages({ locale });

  // Fetch blogs related to this page automatically
  const relatedBlogs = await getRelatedBlogs('/pharma-cosmetics-pet-blowing', locale);
