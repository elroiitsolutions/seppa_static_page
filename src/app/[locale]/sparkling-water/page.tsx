...
export default async function SparklingWaterPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'sparklingwater' });
  const messages = await getMessages({ locale });

  const contentBlocksRaw = t.raw('contentBlocks') as any[];
  const contentBlocks = contentBlocksRaw.map((block: any, index: number) => {
    const base: any = {
      title: block.title,
      paragraphs: block.paragraphs
    };
    if (index === 0) {
      base.image1 = img1.src;
      base.reverse = true;
    }
    if (index === 1) {
      base.bgClass = 'bg-white';
      base.layout = 'stacked';
      base.image1 = meth4.src;
    }
    return base;
  });

  // Fetch blogs related to this page automatically
  const relatedBlogs = await getRelatedBlogs('/sparkling-water', locale);
